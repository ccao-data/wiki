...
Open data upload can be automated using Socrata's SODA API and the [RSocrata package](https://github.com/Chicago/RSocrata).Choose a schedule that ensures the data is current but that doesn't needlessly pull data from AWS and run up operating expenses. If the data changes less frequently it should likely be updated manually (i.e. shapefiles). 
