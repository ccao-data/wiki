...
2. 288 data is used to update characteristics for sales of parcels with an HIE that started before the switch to iasWorld handling of HIEs but has not yet expired for modeling or performing sales ratio studies. We expect these HIEs to be fully expired by ***XXX***. This is because a property's sale price during this timeframe will reflect the value of its *actual* characteristics (i.e. those stored by the 288), not the value of the frozen characteristics used for assessment. When training [the model](https://github.com/ccao-data/model-res-avm) and evaluating its performance, training data is updated to include 288 characteristic updates for the appropriate timeframe.
